...
st.markdown("Use behavior data below to predict performance level and at-risk status.")
...
with st.form("predict_form"):
    ...
    announcements = st.slider("Announcements Viewed", 0, 100, 15)
    discussion = st.slider("Discussion Participation", 0, 100, 10)
    absence = st.radio("Student Absence Days", ["Under-7", "Above-7"])
    ...
    if submitted:
        ...
        }])[features_to_include]  # ✅ Ensure correct order

        # === Predict
        pred_class = model.predict(input_df)[0]
        pred_proba = model.predict_proba(input_df)[0]

        # === Map class to label + color
        label_map = {
            0: ("Low", "#d9534f"),     # red
            1: ("Medium", "#f0ad4e"),  # orange/yellow
            2: ("High", "#5cb85c")     # green
        }
        ...
        <div style='
            ...
            text-align: center;
            font-size: 18px;'>
            🎯 Predicted Performance Level: {label}
        </div>
        """, unsafe_allow_html=True)

        st.caption(f"Class Probabilities (Low/Medium/High): {np.round(pred_proba, 2)}")

        # === At-risk flag (manual rule-based)
        risk_flag = (
            sum([
        ...
        if risk_flag:
            st.error("⚠️ Predicted At-Risk Student (based on behavioral conditions)")
        else:
            st.success("✅ Student not predicted to be At-Risk")
